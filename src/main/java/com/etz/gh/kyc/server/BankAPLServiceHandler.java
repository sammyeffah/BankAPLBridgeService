...
public class BankAPLServiceHandler implements HttpHandler {
    ...
    public void handleRequest(HttpServerExchange exchange) throws Exception {
        ...
        try {
            ...
            if (isIPAllowed(exchange)) {
                ...
                if (action != null && phone != null && action.equals("dofetchaplservice")) {
                    LOG_PREFIX = action + "***" + phone + " ";
                    System.out.println("PARAM FROM APL>>> " + action + "::" + phone);
                    logger.info("PARAM FROM APL>>> " + action + "::" + phone);
                    String resp;
//                    for Sybase
//                    resp = new GIPProcessor().getAccountProfile(phone);

//                    for MySQL
                    resp = new GIPProcessor().getAccountProfileMySQL(phone);
                    System.out.println("RESPONSE FROM APL>>> " + resp);
                    logger.info("RESPONSE FROM APL>>> " + resp);
                ...
                } else if (action != null && action.equals("doapllookup") && account != null) {
                    LOG_PREFIX = action + "***" + phone + " ";
                    System.out.println("PARAM FROM APL WITH ACCOUNT>>> " + action + "::" + account);
                    logger.info("PARAM FROM APL>>> " + action + "::" + account);
                    String resp;
//                    for Sybase
//                    resp = new GIPProcessor().getAccountProfileSybase(account);
                    
                    //                    for Sybase For BT GCB
                    resp = new GIPProcessor().getAccountProfileSybaseWithGhanaCard(account);
